...
class Conjunto <T>{
  ...
  Conjunto[] cartesiano(Conjunto outro_conjunto) {
    Conjunto[] par = new Conjunto[this.tamanho*outro_conjunto.contar_elementos()];

    int tam_pares = 0;
    for(int i = 0; i<this.tamanho; i++){
      for(int j = 0; j < outro_conjunto.contar_elementos(); j++){
        par[tam_pares] = new Conjunto<T>(2);
        par[tam_pares].inserirNoConjunto(this.conjunto[i]);
        par[tam_pares].inserirNoConjunto(outro_conjunto.getConjunto()[j]);
        tam_pares++;
      }
    }
    return par;
  ...
  void potencia(){
    long tam_potencia = (long)Math.pow(2, this.tamanho);
    for(int i = 0; i < tam_potencia; i++) {
        System.out.print("(");
        for(int j = 0; j < this.tamanho; j++) {
          if((i & (1 << j)) > 0) {
             System.out.print(this.conjunto[j]);
             if(j < this.tamanho-1)
              System.out.print(", ");
          }
